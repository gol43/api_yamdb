...
# Здесь для каждого файла CSV мы сначала формируем список объектов модели
# model_objects),
# а затем используем метод bulk_create, чтобы создать все объекты одним
#  запросом. Весь
# процесс выполняется внутри менеджера контекста transaction.atomic().

# Такой подход позволяет гарантировать атомарность операции создания
# объектов при
# работе с базой данных, а также обеспечивает более высокую производительность
# при работе
# с большим количеством данных.

# Мы уже честно не знаем, что писать
